...
# v0.6.0 B+ / v0.6.1(2026-06-23 PM 拍「允许 js」):HTML 网页确定性生成工具。模型把
# 完整页面 HTML 写进 `html` 字段;**允许 <script>/<style>/图表库(chart.js 等)**——
# 下载件在用户自己浏览器以 file:// 打开(隔离 origin,够不到我方应用),渲染器不再消毒。
# 仅放宽 generated HTML 文件,不动我方任何应用内渲染路径(见 html_generator 模块注释)。
GENERATE_HTML_TOOL: dict[str, Any] = {
    "type": "function",
    "function": {
        "name": "generate_html",
        "description": (
            "生成一个**完整、内容充实、可直接双击打开**的单文件网页 (.html),HTML 写进 `html` 字段。\n"
            "\n"
            "**这是一个真正的网页,不是占位**——你必须像写一个完整作品一样投入,写出打开就能看的成品页面:\n"
            "- 输出 `<!DOCTYPE html>` 开头的**完整文档**(<head> 标题 + <style> 样式 + <body> 内容)。\n"
            "- 用户要「可视化 / 看板 / dashboard / 图表」→ **必须用图表库(chart.js)画真实交互图表**:在 <head> 引入 "
            "`<script src=\"https://cdn.jsdelivr.net/npm/chart.js\"></script>`,<body> 放 <canvas>,末尾 <script> "
            "用 `new Chart(...)` 填入数据初始化。柱/折线/饼按数据选。\n"
            "- **用户没给具体数据时,用合理的示例数据把图表/表格填满**,绝不留空。\n"
            "- **严禁只写一个标题或几行空壳** —— 那是失败的输出;要写出有图表 / 卡片 / 表格 / 完整排版的成品。\n"
            "- 自包含、样式美观克制(可用主色 #008042);中文内容直接写中文。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "title": {"type": "string", "description": "网页标题(用作文件名)。"},
                "html": {
                    "type": "string",
                    "description": (
                        "**完整的网页 HTML 源码**(`<!DOCTYPE html>` 起的完整文档,通常几十到上百行):含 "
                        "<head>(标题 + 图表库 CDN + <style>)+ <body>(图表 <canvas> / 表格 / 卡片等真实内容)"
                        "+ <script>(初始化图表、填入数据)。**不要只写标题或几行 —— 要能打开就看到完整看板。**"
                    ),
                },
            },
            "required": ["title", "html"],
...
- **网页 / HTML / 单页展示 / 可视化看板 / dashboard** → `generate_html`(写完整网页;可视化用图表库画真图)。

【铁律(所有生成工具通用)】
- pptx/xlsx/docx/csv:你**只产出结构化内容数据**(大纲 / 表格数据 / 文档结构);**绝不写**渲染/排版/样式代码,版式由渲染器统一控制。
- **generate_html 例外**:你**亲手写一个完整、内容充实的网页**(`<!DOCTYPE html>` 完整文档,含 <head> 引图表库 + <body> 内容 + <script> 初始化)。用户要「可视化/看板」时**必须用 chart.js 等画真实交互图表**(柱/折线/饼),没数据就编合理示例数据填满;**绝不能只回一个标题或几行空壳**——要写出打开就能看到图表的成品页面。
