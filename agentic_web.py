...
GENERATE_HTML_TOOL: dict[str, Any] = {
    ...
    "function": {
        "name": "generate_html",
        "description": (
            "生成一个可下载的单文件网页 (.html)。把**完整页面 HTML** 写进 `html` 字段。\n"
            "\n"
            "**关键约束**:\n"
            "- 可写**完整文档**(<!DOCTYPE html><html><head>…<body>…),也可只写正文片段(渲染器会套外壳)。\n"
            "- **允许 <style> 和 <script>**:需要图表/交互时,引入图表库(如 "
            "`<script src=\"https://cdn.jsdelivr.net/npm/chart.js\"></script>`)并写初始化脚本。\n"
            "- 用户要「可视化 / 看板 / dashboard」时,**优先用图表**(柱/折线/饼)呈现数据,而非纯文字表格。\n"
            "- 自包含、可直接双击打开;样式美观克制;中文内容直接写中文。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "title": {"type": "string", "description": "网页标题(用作文件名;片段模式下也作页面 H1)。"},
                "html": {
                    "type": "string",
                    "description": "完整页面 HTML(或正文片段)。可含 <style>/<script>/图表库。",
                },
            },
            "required": ["title", "html"],
        },
...
- **generate_html 例外**:你**直接写网页 HTML**(可含 <style>/<script>/图表库)。用户要「可视化/看板」时**优先用图表**(chart.js 等 CDN 库画柱/折线/饼)呈现数据,别只给纯文字表格;页面自包含、可双击打开。
