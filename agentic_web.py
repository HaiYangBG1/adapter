...
# v0.6.9 B11 大文件分多步:pptx/xlsx/docx 共用的「分片」schema 字段。内容超单次上限
# 时,模型把任务拆成 N 片多次调用本工具(part_index 1→total_parts),渲染器累积、末片
# 合并出一份完整文件。一次能出完则省略 / 填 1(单次行为零回归)。
_PART_INDEX_SCHEMA: dict[str, Any] = {
    "type": "integer",
    "description": "(可选,仅内容大到一次出不完时用)当前是第几部分,从 1 开始;一次出完则省略或填 1。",
}
_TOTAL_PARTS_SCHEMA: dict[str, Any] = {
    "type": "integer",
    "description": (
        "(可选,仅内容大到一次出不完时用)总共分几部分。当内容超过单次上限"
        "(PPT 单次≤30 页 / Excel 单次≤5000 行 / 文档单次≤60 节)时,把内容**拆成 N 片**"
        "多次调用本工具:part_index 从 1 递增到 total_parts,每片塞满但不超单次上限,"
        "渲染器自动把所有片合并成一份完整文件、在末片(part_index==total_parts)时才产出下载。"
        "一次能出完就**省略本字段或填 1**。"
    ),
}
...
GENERATE_PPTX_TOOL: dict[str, Any] = {
    ...
    "function": {
        ...
        "description": (
            ...
            "- 建议 6-15 页(含封面);单次最多 30 页,**更大的演示用 total_parts 分多步出**(见下)。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "part_index": _PART_INDEX_SCHEMA,
                "total_parts": _TOTAL_PARTS_SCHEMA,
            },
...
# v0.6.x B14(文件能力优化):Markdown / 纯文本生成。模型直接在 `body` 写内容
# (markdown / 纯文本是*内容*非渲染代码,合 A 铁律),也可退化用 docx 式 sections。
# 纯 stdlib 渲染(md_generator / txt_generator),单片(不进 _FILE_GEN_PART_KEYS)。
GENERATE_MD_TOOL: dict[str, Any] = {
    "type": "function",
    "function": {
        "name": "generate_md",
        "description": (
            "生成可下载的 **Markdown (.md)** 文件。适合 笔记 / 说明 / README / 技术文档 / 带格式的纯文本。\n"
            "你**直接在 `body` 里写完整的 markdown 正文** —— 支持 # 标题、- 列表、表格、**加粗**、`代码`、代码块等。\n"
            "标题写进 `title`(用作文件名);正文写进 `body`。这是模型最擅长的格式,放手写。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "title": {"type": "string", "description": "文档标题(用作文件名),如「项目说明」。"},
                "body": {"type": "string", "description": "完整的 markdown 正文(可含标题/列表/表格/代码块等标记)。"},
            },
            "required": ["title", "body"],
        },
    },
...
GENERATE_TXT_TOOL: dict[str, Any] = {
    "type": "function",
    "function": {
        "name": "generate_txt",
        "description": (
            "生成可下载的**纯文本 (.txt)** 文件。适合纯文字记录 / 会议纪要 / 清单 / 无格式导出。\n"
            "在 `body` 里写纯文本正文(**不要** markdown 标记 —— 要带格式用 generate_md);换行即换行。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "title": {"type": "string", "description": "标题(用作文件名)。"},
                "body": {"type": "string", "description": "纯文本正文(保留你写的换行)。"},
            },
            "required": ["title", "body"],
        },
    },
...
# v0.6.x B14:+ generate_md / generate_txt(纯 stdlib,单片)。
...
FILE_GEN_TOOL_META: dict[str, dict[str, str]] = {
    ...
    "generate_md": {"kind": "md", "ext": "md", "mime": "text/markdown; charset=utf-8", "default_name": "笔记", "preview": "none"},
    "generate_txt": {"kind": "txt", "ext": "txt", "mime": "text/plain; charset=utf-8", "default_name": "文本", "preview": "none"},
...
【关于 depends_on】
- **下方【数据集结构】已经给出真实的表名 / 列名 / 类型 / 取值样例** —— 直接照着用**真实列名**写每个 step 的 question,**不要再列「查看表结构 / 列出所有表名列名」这类探查 step**(结构已经在你眼前,再查一遍是浪费,还会拖慢整个 plan)。
- **绝大多数业务查询互相独立**,`depends_on` 填空数组 `[]` 并行最快。错误示例:「① 算全国总营业额,② 算各区域占比」—— 可并行,② 不需要等 ①(② 本身就能算"区域营业额 / SUM(全国)")。
- ⚠ **唯一例外**:若下方【数据集结构】明确写了"未预取到 / 不全",才回到兜底办法 —— 先列一个「查看表结构」step 放在 plan 第一个,再给所有"需要这些表名/列名才能写出来"的查询 step 填上 `depends_on: ["该结构 step 的 id"]`,否则查询 step 在不知道结构时盲猜表名列名,在多表数据集里反复试错、撞超时(实测教训:十几张利润表的数据集,查询 step 不依赖结构 step → 全部超时)。**结构已给出时(常态)绝不要这么做,直接并行。**
...
- 表名 / 列名 / 类型已在下方【数据集结构】给出,直接用真实列名写 question 即可,不必再为"看结构"单列 step。
...
- **Markdown / 笔记 / README / 技术文档 / 带格式纯文本** → `generate_md`(直接在 `body` 写完整 markdown)。
- **纯文本 / 纯文字记录 / 无格式导出** → `generate_txt`(`body` 写纯文本)。
...
- **内容太大、一次填不下时**(PPT 超 30 页 / 表超 5000 行 / 文档超 60 节):用 `total_parts` 把内容**分多步**出 —— 同一个生成工具连续调用 N 次,`part_index` 从 1 递增到 `total_parts`,每次只放这一片的 slides/sheets/sections(塞满但不超单次上限),系统会自动把所有片合并成一份完整文件。**一次能出完就别用 part_index/total_parts**。
...
- **禁止只在 content 里宣告「我来为您生成…」「这就帮你做一个…」却不 emit tool_call** ——
  这种"只说不做"会让用户干等几十秒后什么都拿不到,是最严重的失败。要生成就**直接 emit
  工具调用**,不写任何过渡叙述;确实不需要文件,才用文字正常作答。
- 禁止输出 <tool_call>、{"name":...} 这类原始语法 —— 直接 emit 标准 tool_calls。
"""


# v0.6.6 B9(「生成文件」force 单开关):force 模式系统提示词。与 FILE_GEN_PROMPT 不同 ——
# 用户已通过「生成文件」chip **明确要一个文件**,且 tool_choice=required 已在协议层强制
# 必调工具,所以这里**去掉「先判断要不要生成」整段**,只留「选哪种类型 + 填全参数」,
# 防模型在 reasoning 里纠结「要不要生成」浪费 token / 动摇(force 轮 thinking 已关,
# 但 prompt 也不该再留犹豫空间)。
FILE_GEN_FORCE_PROMPT = """你是一个能把内容整理成**可下载文件**的助手。用户已明确要你生成一个文件,系统会强制你调用其中一个生成工具——你的任务**只是判断要哪种文件、并把内容填全**,不要纠结要不要生成。
...
- **纯文本 / 纯文字记录 / 无格式导出** → `generate_txt`(`body` 写纯文本)。
- **网页 / HTML / 单页展示 / 可视化看板 / dashboard** → `generate_html`(只填 `brief` 一段话讲清需求+数据+想要的图表,系统会生成带交互图表的完整网页;**别自己写 HTML 代码**)。
- 用户没明说类型时,挑最贴合内容的一种(如「Q4 营收复盘」→ pptx;「各门店销售」→ xlsx)。

【铁律(所有生成工具通用)】
- 所有生成工具:你**只填工具参数**(结构化数据 / 或网页 brief),**绝不写**渲染/排版代码;版式与最终文件由系统生成。
- generate_html:只在 `brief` 里**用一段话讲清需求 + 数据 + 想要的图表**(系统会据此自由生成完整含 chart.js 的网页);**别自己写 HTML/JS 代码**。
- 选定一种工具,**一次性把内容填全**,emit 这**一个** tool_call(同一轮不要同时调多个生成工具;一次请求出一份文件)。
- **内容太大、一次填不下时**(PPT 超 30 页 / 表超 5000 行 / 文档超 60 节):用 `total_parts` 把内容**分多步**出 —— 同一个生成工具连续调用 N 次,`part_index` 从 1 递增到 `total_parts`,每次只放这一片的 slides/sheets/sections(塞满但不超单次上限),系统会自动把所有片合并成一份完整文件。**一次能出完就别用 part_index/total_parts**。
- 工具调用成功后系统会把文件以"文件卡"展示在对话下方供用户下载;你随后用一两句话告知用户即可,不要把全部内容再复述一遍。
- 信息不足就给合理的框架性内容,不要编造具体数字。
...
【严禁】
- 禁止用自然语言把文件内容写在 content 里冒充"已生成"——必须 emit 工具调用。
- **禁止只在 content 里宣告「我来为您生成…」却不 emit tool_call** —— 系统已强制你必须
  调用生成工具,直接 emit,不写任何"我来…""这就…"之类过渡叙述。
- 禁止输出 <tool_call>、{"name":...} 这类原始语法 —— 直接 emit 标准 tool_calls。
"""


# v0.6.10 B12(测试域 live FAIL 修):「分析上传的表 + 做成看板/图表/报表」组合模式。
# 既挂 excel_query(查真实数据)又挂全部 generate_*(出文件)—— 必须**先查表再生成**,
# 否则看板里的数字全是编的(测试域实证:73219/48571 等真实值一个没进图)。
EXCEL_FILE_GEN_PROMPT = """你既能**查询用户上传的表格**(`excel_query` 工具),也能**生成可下载文件 / 可视化看板**(generate_* 工具)。用户上传了表格数据集,并希望基于它的真实数据做出文件或看板。

【🔴 铁律:看板/图表/报表里的数字必须来自表,严禁编造】
- 要做**基于这张表的图表 / 看板 / dashboard / 可视化 / 报表**:**必须先用 `excel_query` 查出要展示的真实数据**(具体分类 + 具体数值),拿到真实结果后,**再调 `generate_html`**(或 generate_xlsx/generate_pptx 等),并在 brief / 内容里**逐条写清 excel_query 查到的真实数字**。
- **绝不允许没查表就直接生成图表** —— 那样图里的数字全是编的,是最严重的失败。
- 一次 excel_query 不够就多查几次(各维度 / 各指标),把要画进图的数据查全,再生成。

【流程】
1. 先 `excel_query`:问出图表需要的真实数据(如「各门店营业额」「各月销售额」)。需要就多查几次。
2. 数据齐了 → 调 `generate_html` 把这些**真实数值**做成带 chart.js 的看板(brief 里逐条写明真实数字),或 generate_xlsx / generate_pptx 等。
3. 文件生成后系统会把文件卡展示在对话下方,你用一两句话告知用户即可,别复述全部内容。

【其它】
- 只是问数据 / 要分析结论(不要文件)→ 正常 `excel_query` 查完文字作答,不必生成文件。
- generate_html 只在 `brief` 里用一段话讲清需求 + **真实数据**,别自己写 HTML/JS 代码。
- 所有生成工具:你只填工具参数(结构化数据 / 网页 brief),绝不写渲染代码。
"""
...
class AgentConfig:
    ...
    # v0.6.6 B9(「生成文件」force 单开关):True 时首轮(及 intent-leak 续轮)用
    #   tool_choice="required" 而非指定工具名 —— 挂全部 generate_*,模型**只判类型、
    #   强制必调其一**(治 narrate:auto 模式 ~20-30% 只回「我来生成…」不调工具)。
    #   与 force_first_tool_name(强制单一工具)互斥:那个发指定 function name,
    #   这个发 "required"(任一工具)。adapter.py 在 gen_file_force 请求时置 True。
    #   🔴 K2.6/vLLM0.18 实测支持 "required"(老 SGLang 0.5.9「指定工具名比 required
    #   稳」顾虑对新引擎过时,见 v0.2.25 runbook);但 force 轮必须关 thinking
    #   (intermediate_thinking_enabled 默认 False 已满足),否则 thinking 吃光
    #   token 撞 length 出不来 tool_call(实测:开 thinking 时 reasoning 2500 token
    #   还没 emit 就 finish_reason=length)。
    force_required_tool: bool = False

    # v0.6.8 viz/file-gen narrate 修(A):file_gen **auto** 模式专用兜底。首轮仍
    #   tool_choice=auto(让模型自决要不要生成文件,不硬出);但**一旦本轮 content 是
    #   「我来为您生成…」这类 narrate-then-stop 叙述、没真 emit tool_call**,intent-leak
    #   续轮就强制 tool_choice="required" 逼出文件。与 force_required_tool 的区别:那个
    #   **首轮就** required(B9「生成文件」force chip,用户已明确要文件);这个只在**检测
    #   到 narrate 之后**才 required,首轮保留 auto 的「可以不生成」语义(只聊天/分析时
    #   不硬出文件)。adapter.py file_gen auto 分支置 True。**兼作「当前是 file_gen auto
    #   模式」标记** → 驱动 _looks_like_file_gen_narrate 叠加检测(见 run_agent_stream)。
    force_required_on_intent_leak: bool = False
    ...
    # B12:第 4 参 context_messages = 当前对话 messages,给 generate_html 看板用真实数据。
    file_renderer: Optional[Callable[..., dict]] = None
...
# v0.6.8 viz/file-gen narrate 修(A):file_gen **auto** 模式专用 narrate 检测。
# 背景:全局 _INTENT_LEAD_PHRASES / _INTENT_NARRATE_RE 是给 web/excel「查询/分析」
# 场景调校的 —— 引导词无「我来」、动作词只有 查询/分析/调用/获取… —— 完全漏掉
# file_gen 场景特有的「我来为您生成一个 Excel…」这类 narrate-then-stop(auto 模式
# 实测 ~20-30% 只叙述不 emit tool_call)。单独一个收敛正则,**只在 file_gen auto
# 模式(cfg.force_required_on_intent_leak)由调用方叠加,不碰全局检测器 → web/excel/
# pptx-force 零回归**。
# 只看**开头 40 字**:narrate-then-stop 的特征是开门见山宣告「我来生成…」;只看头部
# 天然避开长答案句中的「…如果需要我可以生成汇总表」这类**提议/条件句**(那是在征询、
# 不是 leak),不误伤。动作词**故意不含「分析」**(那是"解读已有文件"的正常作答,
# 不该被强制出文件)。
_FILE_GEN_NARRATE_RE = re.compile(
    r"(我来|我这就|这就|我现在|现在就|我马上|马上|让我|我先|我去|我帮你|我帮您|稍等|我准备)"
    r"[^。\n]{0,14}"
    r"(生成|制作|做一?[个份]|做成|导出|创建|整理成|帮你?(?:做|生成|整理)|写一?[个份])"
    r"[^。\n]{0,10}"
    r"(文件|表格|表|excel|xlsx|ppt|pptx|幻灯|演示|文档|word|docx|网页|html|"
    r"页面|看板|仪表盘|dashboard|报告|图表|可视化|csv)",
    re.IGNORECASE,  # 模型常输出 Excel / PPT / HTML / Word 等首字母或全大写英文名词
)

# 征询/正反问句(「要不要我帮你做个表?」)是**完整的交互回合** —— 用户知道回「要」
# 即可,不是 narrate-then-stop 的"宣告动作后戛然而止、干等无果"。命中此前缀 → 不判
# leak、不强制出文件(尊重模型在信息不足时问一句)。注意"要不"(="那么")不排除,
# 那是口语宣告(「要不我来给你做个表」=好的我来做)。
_FILE_GEN_OFFER_RE = re.compile(r"(要不要|需不需要|是否需要|想不想|用不用|要我帮)")


def _looks_like_file_gen_narrate(text: str) -> bool:
    """file_gen auto 模式:模型在 content 里宣告「我来为您生成一个 Excel…」却没真
    emit tool_call(narrate-then-stop)。只看开头 40 字,避开句中提议/条件句 + 征询问句。
    仅在 cfg.force_required_on_intent_leak(file_gen auto)时由调用方叠加 —— 命中 →
    intent-leak 续轮强制 tool_choice=required 逼出文件。"""
    t = (text or "").strip()
    if not t:
        return False
    head = t[:40]
    if _FILE_GEN_OFFER_RE.search(head):
        return False
    return bool(_FILE_GEN_NARRATE_RE.search(head))
...
def _finalize_answer(
    ...
    needs_synthesis = (
        ...
        # v0.6.8(A):file_gen auto 非流式终结路径,同样叠加 file_gen narrate 检测,
        # 否则「我来为您生成…」漏判 → 直接当 final answer 返回(更糟)。命中走合成兜底
        # (产出文字结论;非流式路径无续轮、出不了文件,但比 narrate 戛然而止强)。主路
        # 径(file_gen 流式)在 run_agent_stream 续轮强制 required 真出文件,见那里。
        or (cfg.force_required_on_intent_leak
            and _looks_like_file_gen_narrate(stripped))
    )
    if needs_synthesis:
...
def run_agent_stream(
    ...
    # v0.6.9 B11 分多步:非 None 时表示「正在分多步累积一份文件」,holds 续片要用的
    # artifact_id + 类型元数据。末片 emit ready 后清回 None;模型提前停(无末片)时由
    # 下方「无 tool_call 应答分支」兜底 finalize。
    file_gen_pending: Optional[dict[str, Any]] = None
    ...
    for iteration in range(1, cfg.max_iterations + 1):
        ...
        # ⚠️ _force_this_turn=True 只是"本轮**有资格**强制工具"的候选闸,**不等于**真会
        #    force —— 实际是否注入 tool_choice 由下面各分支的 cfg.force_* 条件决定。
        #    例:file_gen auto 首轮 _force_this_turn=True 但两个 force 条件都不满足
        #    (force_required_tool=False、force_tool_choice_next 首轮=False)→ 保持 auto。
        # v0.6.6 B9(「生成文件」force 单开关):tool_choice="required" —— 挂全部
        # generate_*,模型**只判类型、强制必调其一**(治 narrate)。与下面 named-tool
        # 分支(force_first_tool_name 强制单一工具)互斥;此分支优先。K2.6/vLLM0.18
        # 实测支持 "required",force 轮 thinking 已由 _build_iteration_extra 关。
        if (
            _force_this_turn
            and not is_last
            and (
                cfg.force_required_tool
                # v0.6.8(A):file_gen auto **续轮**(intent-leak narrate 命中后)也走
                # required。首轮 force_tool_choice_next=False → 不进 → 保持 auto 自决;
                # narrate 命中后 force_tool_choice_next=True → required 逼出文件。与上面
                # force_required_tool(B9 首轮就 required)互补,文案走下方续轮分支。
                or (cfg.force_required_on_intent_leak and force_tool_choice_next)
            )
            and current_tools
        ):
            ...
            if force_tool_choice_next:
                _force_msg = (
                    f"intent-leak 续轮:强制必调生成工具(required,只判类型)"
                    f"(已续 {intent_leak_retries_used} 次)"
                )
                force_tool_choice_next = False  # 一次性,用完立即清
            else:
                _force_msg = "首轮强制必调生成工具(required,模型只判类型;防仅叙述不调工具)"
            progress_cb(
                "agent_first_turn_tool_forced",
                _force_msg,
                {"iteration": iteration, "tool_choice": "required"},
            )
            yield from _drain_queue()
        elif (
            _force_this_turn
        ...
        if path == "content":
            ...
            # ── v0.6.9 B11 分多步兜底:模型在累积中途**停下直接作答**(没发末片,
            #    auto 模式偶发)→ 用已暂存分片 finalize 出文件,避免占位卡永远转圈。
            #    force 模式 tool_choice=required 不会走到这里(每轮必调工具)。
            _just_finalized_file = False
            if file_gen_pending is not None and cfg.file_renderer is not None:
                _fp = file_gen_pending
                file_gen_pending = None
                _just_finalized_file = True
                try:
                    fin = cfg.file_renderer(
                        _fp["tool_name"], {}, _fp["artifact_id"], None, 1, 1, True)
                    if not isinstance(fin, dict):
                        fin = {"ok": False, "error": "renderer 返回非 dict"}
                except Exception as exc:  # noqa: BLE001
                    fin = {"ok": False, "error": f"{type(exc).__name__}: {exc}"}
                if fin.get("ok"):
                    _art = {
                        "id": _fp["artifact_id"], "type": "file", "kind": _fp["kind"],
                        "name": fin.get("name") or _fp["name"],
                        "mime": fin.get("mime") or _fp["mime"],
                        "status": "ready", "downloadUrl": fin.get("download_url"),
                        "previewKind": _fp["preview"],
                    }
                    if fin.get("size") is not None:
                        _art["size"] = fin["size"]
                    yield _sse_artifact_chunk(model, _art)
                else:
                    yield _sse_artifact_chunk(model, {
                        "id": _fp["artifact_id"], "type": "file", "kind": _fp["kind"],
                        "name": _fp["name"], "mime": _fp["mime"], "status": "error",
                        "previewKind": _fp["preview"],
                        "error": str(fin.get("error") or "分多步合并失败"),
                    })
                progress_cb(
                    "agent_file_gen_part_finalize",
                    "模型未发末片即作答 → 已用暂存分片合并出文件",
                    {"tool": _fp["tool_name"], "ok": bool(fin.get("ok"))},
                )
            ...
            looks_intent = (
                ...
                # v0.6.8(A):file_gen auto 专用 —— 全局检测器漏「我来为您生成…」,叠加
                # file_gen narrate 检测。命中 → 下方 can_intent_retry → 续轮 required。
                or (cfg.force_required_on_intent_leak
                    and _looks_like_file_gen_narrate(content_buf))
            )
            can_intent_retry = (
                looks_intent
                and not is_last
                # B11:本轮刚用暂存分片 finalize 出过文件 → 别再 intent-retry 触发二次生成。
                and not _just_finalized_file
        ...
        if path == "tool_calls":
            ...
            if not assembled:
                ...
                if _file_tc_found is not None:
                    ...
                    # B11 分多步:取 part_index/total_parts(generic int;非分多步默认 1/1)。
                    try:
                        part_index = int(file_args.get("part_index") or 1)
                    except (TypeError, ValueError):
                        part_index = 1
                    try:
                        total_parts = int(file_args.get("total_parts") or 1)
                    except (TypeError, ValueError):
                        total_parts = 1
                    # 续片复用同一 artifact_id(前端同卡 generating→ready);首片/单次新建。
                    if file_gen_pending is not None:
                        artifact_id = file_gen_pending["artifact_id"]
                    else:
                    ...
                    #    续片复用同一张卡不重发,避免闪烁 —— 仅首片/单次发。
                    if file_gen_pending is None:
                        yield _sse_artifact_chunk(model, {
                            "id": artifact_id,
                            "type": "file",
                            "kind": f_kind,
                    ...
                    if ok and render_result.get("partial"):
                        ...
                        partial_tool_result = {
                            ...
                            "partial": True,
                            "note": (
                                f"已收到第 {received}/{total_parts} 部分并暂存(尚未出文件)。"
                                f"请**继续** emit 同一个 {file_tool_name},part_index={received + 1}、"
                                f"total_parts={total_parts},放入下一片内容;直到 part_index 到 "
                                f"{total_parts}(末片)系统才合并全部、产出最终文件。"
                            ),
                        }
                        progress_cb(
                            "agent_file_gen_part",
                            f"已收到第 {received}/{total_parts} 部分,等待续片",
                            {"received": received, "total": total_parts, "tool": file_tool_name},
                        )
                        yield from _drain_queue()
                        augmented = augmented + [
                            {"role": "assistant", "content": "", "tool_calls": [file_tc]},
                            {"role": "tool", "tool_call_id": file_tc_id,
                             "content": json.dumps(partial_tool_result, ensure_ascii=False)},
                        ]
                        continue  # 不收尾,续轮出下一片
                    # 末片 / 单次:清 pending(下方按 ready/error 正常收尾 + 置 dispatch_done)。
                    file_gen_pending = None
